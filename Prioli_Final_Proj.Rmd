...
rm(list = c("HDIraw", "HDIcolnm", "HDI_vhhd", "HDI_hhd", "HDI_mhd", "HDI_lhd", "HDI_oth", "HDIanti"))
```
...
OECD_wrangle <- function(df, stem){
  ...
  rename(year = TIME) %>%
  rename(!!stem := Value) # %>%         # Tidy eval syntax - use !! to unquote LHS,
  # group_by(country3) %>%              # then := substitute assignment operator
  # mutate(maxyr = max(year)) %>%
  # ungroup() %>%
...
# Excluding countries that have no data
