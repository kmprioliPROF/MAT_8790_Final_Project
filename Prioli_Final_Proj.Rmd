...
```{r import, include = FALSE, echo = TRUE}
...
rm(list = c("HDIraw", "HDIcolnm", "HDI_vhhd", "HDI_hhd", "HDI_mhd", "HDI_lhd", "HDI_oth"))
...
rm(list = c("SPI_2017_raw", "SPI_2017_data", "SPI_2016_raw", "SPI_2016_data", "SPI_2015_raw", "SPI_2015_data", 
   "SPI_2014_raw", "SPI_2014_data", "SPI_join"))
...
#### World Economic Forum Gender Equality Indices data ----
...
GDPdata <- GDPsub %>% 
  ...
  mutate(maxyr = max(year)) %>% 
  mutate(maxyr = case_when(
    is.na(maxyr) == TRUE ~ as.double(TRUE),
    maxyr == year ~ 1,
    maxyr != year ~ 0
  )) %>% 
  ungroup()
...
rm(list = c("GDPraw", "GDPsub", "GDPcolnames"))
...
rm(list = c("hoursworkedraw", "hoursworkeddata", "incomeineqraw", "incomeineqdata", "infantmortraw",
            "infantmortdata", "oophcspendraw", "oophcspenddata", "tertiaryedraw", "tertiaryeddata", "OECDjoin"))
