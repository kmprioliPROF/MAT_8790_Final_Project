...
<!-- Notes
  * Limited to 5 years (2013 - present; probably want to examine final dataset and exclude 2018 - mostly NA)
...
# Creating a framework with one row for every country/year combination

countriesjoin <- bind_rows(countries, countries, countries, countries, countries)
countrylen <- dim(countries)[[1]]
years <- as.tibble(rep(c(2013, 2014, 2015, 2016, 2017),
                       c(rep(countrylen, 5))))
colnames(years) <- "year"
countriesjoin <- bind_cols(countriesjoin, years)

# Joining datasets together

joindata_1 <- left_join(countriesjoin, HDIdata, by = c("country", "year"))
joindata_2 <- left_join(joindata_1, SPIdata, by = c("country3", "year"))
joindata_3 <- left_join(joindata_2, WHRdata, by = c("country", "year"))
joindata_4 <- left_join(joindata_3, genderdata, by = c("country", "year"))
joindata_5 <- left_join(joindata_4, lifeexpdata, by = c("country", "year"))
joindata_6 <- left_join(joindata_5, GDPdata, by = c("country3", "year"))
joindata_7 <- left_join(joindata_6, OECDdata, by = c("country3", "year"))
View(joindata_7)
...
# Next step - create a sensible subset of the data for analysis
```
