...
First, the top and bottom 20 countries were compared by HDI:

```{r HDI_country, include = TRUE, echo = TRUE, fig.height = 6, fig.width = 6}
alldata_HDI <- alldata %>%
  filter(!is.na(HDIindex) == TRUE) %>% 
...
The United States ranks twentieth by social progress index.

Next, world happiness index:
...
WHR_country_point <- ggplot(data = alldata_WHR_40, 
  ...
  guides(color = FALSE) +
  xlim(0, 10) +
  xlab("World Happiness") +
  ylab("Country") +
  ggtitle("World Happiness by Country, \nTop and Bottom 20 Countries")
WHR_country_point
```
...
```{r gender_country, include = TRUE, echo = TRUE, fig.height = 6, fig.width = 6}
...
```{r lifeexp_country, include = TRUE, echo = TRUE, fig.height = 6, fig.width = 12}
...
Exploring GDP by country:

```{r GDP_country, include = TRUE, echo = TRUE, fig.height = 6, fig.width = 9}
alldata_GDP <- alldata %>%
...
Finally, examining infant mortality:

```{r infantmort_country, include = TRUE, echo = TRUE, fig.height = 6, fig.width = 6}
