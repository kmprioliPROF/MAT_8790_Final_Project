...
alldata <- alldata[c(1:2, 13:14, 7, 12, 3:6, 8:11)]
...
First, the top and bottom 20 countries were compared by World Happiness Score:

```{r WHR_country, include = TRUE, echo = TRUE, fig.height = 6, fig.width = 6}
alldata_WHR <- alldata %>%
...
Next, the social progress index:

```{r SPI_country, echo = FALSE, fig.height = 6, fig.width = 6}
alldata_SPI <- alldata %>%
...
```{r kmeans_happiness_GDP, warning = FALSE, message = FALSE, fig.width = 8, fig.height = 6}
kmdata <- kmdf(clusterdata, "country", "happiness", "logGDP")

set.seed(19811221)
km_happiness_GDP <- kmeans(kmdata[, 2:3], 4)
km_happiness_GDP_cluster <- as.factor(km_happiness_GDP$cluster)

clusterdata_1 <- cbind(clusterdata, km_happiness_GDP_cluster)

km_happiness_GDP_plot <- ggplot(data = clusterdata_1, 
                                aes(x = logGDP, y = happiness, 
                                    color = km_happiness_GDP_cluster,
                                    size = US,
                                    shape = HDI_cat)) +
  geom_point() +
  ylim(0, 10) +
  ...
  ggtitle("Cluster Analysis, World Happiness Score vs. Log-Scale GDP")
km_happiness_GDP_plot
```
